...
impl LevenshteinAutomaton {
    ...
    pub fn step(&self, state:&LevenshteinAutomatonState, cha:char) -> LevenshteinAutomatonState {
        let mut new_state = vec![(state[0]+1).min(self.max_edits+1)];
        println!("{:?}", state);
